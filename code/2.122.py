...
prompt_content = (
    f"请根据以下要求生成报告：\n"
    f"1. 以下是参考信息：\n{md_content}\n"
    f"""2. 严格模仿以下格式：（2）拟使用林地结构
①林地权属
项目拟长期使用林地面积34.3006hm2，均为集体林地，属扶绥县龙头乡腾广村集体所有。林地权属清楚、无争议。
②土地种类
乔木林地面积31.7841hm²，占拟使用林地总面积的92.7%；
采伐迹地面积2.2703hm²，占拟使用林地总面积的6.6%；
特殊灌木林地面积0.2462hm²，占拟使用林地总面积的0.7%。
...
try:
    ...
    output_file = os.path.join(output_dir,  '2.122.txt')
