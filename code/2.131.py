...
keywords = ["按使用林地类型面积蓄积统计"]  # 自定义关键词列表
...
prompt_content = (
    f"请根据以下要求生成报告：\n"
    f"1. 以下是参考信息：\n{md_content}\n"
    f"""2. 严格模仿以下格式：（3）使用林地林木资源现状
项目建设拟使用林地面积34.3006hm²，其中涉及林木面积32.0303hm2，占项目建设拟使用林地总面积93.4%，蓄积量3405m3。
①林木权属
项目拟使用林地林木面积32.0303hm2，蓄积量3405m3。林木权属均为个人所有。
林木权属清楚、无争议。
②林木起源
起源均为人工。
③林种组成
用材林面积31.7841hm²，蓄积量3405m3；
经济林面积0.2462hm²，无蓄积量。
...
try:
    ...
    output_file = os.path.join(output_dir,  '2.131.txt')
