...
def excel_to_md(excel_file, output_dir=OUTPUT_FOLDER, filter_method='default', columns_to_exclude=None, columns_to_keep=None, exclude_patterns=None, target_table_pattern='建设项目使用林地因子调查表'):
    """将单个Excel文件中的所有表转换为Markdown文件，对特定表格应用过滤
    ...
    参数:
        ...
        target_table_pattern (str): 目标表格标题的匹配模式，只对这类表格应用过滤
    """
    # 创建输出目录
    ...
    try:
        ...
        for sheet_name in excel.sheet_names:
            ...
            # 检查是否是"建设项目使用林地因子调查表"，只对这种表格应用过滤
            is_target_table = re.search(target_table_pattern, title) is not None
            
            if is_target_table and len(df) >= 2:
                print(f"  检测到目标表格: {title}，将应用列过滤")
...
def process_all_excel_files():
    """处理指定文件夹中的所有Excel文件，为"建设项目使用林地因子调查表"提供三种过滤方案"""
    # 确保输入文件夹存在
    ...
    print("=== 注意：过滤方案只应用于'建设项目使用林地因子调查表'，其他表格正常处理 ===\n")
